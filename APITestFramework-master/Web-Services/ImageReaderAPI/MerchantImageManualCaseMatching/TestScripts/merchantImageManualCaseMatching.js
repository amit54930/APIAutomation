...
var config_MerchantImageManualCaseMatching = config.MerchantImageManualCaseMatching;
...
eventEmitter.on("testCaseData", function (testCaseData) {
  url = root_url+config_MerchantImageManualCaseMatching.call_suffix_MerchantImageManualCaseMatching;
...
function template1_success(object, testData, url) {
  ...
  url= url.replace("{documentName}", testData.documentName);
  var options = { headers: config.header,qs:{ documentName:testData.documentName} }
