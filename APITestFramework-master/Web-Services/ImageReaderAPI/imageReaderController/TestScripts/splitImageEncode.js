...
var config_ImageReaderControllerAPI = config.ImageReaderControllerAPI;
...
//Web service URL
var root_url = config.image_reader_url;
...
eventEmitter.on("testCaseData", function (testCaseData) {
  url = root_url+config_ImageReaderControllerAPI.call_suffix_toEncode;
...
//  url= url.replace("{caseStageId}", testData.caseStageId);
  var options = {headers: config.header, qs:{ caseNumber:testData.caseNumber,documentTypeId:testData.documentTypeId} }
 ...
 // console.log("Status Code Matched");
  ...
  var queries = Query.generateDynamicQuery(testData.caseStageId);
...
var res=JSON.parse(expected_response);
    //console.log("^^^^^^^^^^^^^^^^^ " +res);
    res[0]['documentName'] = res[0]["document_name"];
    delete res[0]['document_name'];
    res[0]['originalName'] = res[0]["original_document_name"];
    delete res[0]['original_document_name'];
    res[0]['redactedName'] = res[0]["redacted_document_name"];
    delete res[0]['redacted_document_name'];
     //  console.log("++++++++++++++++"+res);
    }
    else {
   ...
   utils.VerifyResponseBody(object,actual2,res, ['base64']);
    } else {
