...
var config_fetchListOfDisputeStagesAPI = config.FetchListOfDisputeStagesAPI;
...
//Web service URL
var root_url = config.servicecasedata_root_url + config_fetchListOfDisputeStagesAPI.call_suffix_tofetchListOfDisputeStages;
 ...
 // console.log(url);
  ...
  if (!queries.length == 0) {
    ...
    var i;
      ...
      res[i]['stageId'] = res[i]["dispute_stage_id"];
      delete res[i]['dispute_stage_id'];
      res[i]["stageName"] = res[i]["dispute_stage_name"];
      delete res[i]['dispute_stage_name'];

      var stageId= parseInt(res[i].stageId);
      res[i].stageId=stageId;
    }
