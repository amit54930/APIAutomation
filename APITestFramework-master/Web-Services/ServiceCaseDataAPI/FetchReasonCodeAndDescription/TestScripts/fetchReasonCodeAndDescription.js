...
var config_fetchReasonCodeAndDescription= config.fetchReasonCodeAndDescription;
...
//console.log("+-+---+--"+JSON.stringify(test_data_TestCaseWise));
//expected Response Root path
var expected_response_root_path = config.expected_response_root_path;

//Web service URL
var url = config.servicecasedata_root_url+config_fetchReasonCodeAndDescription.call_suffix_toFetchReasonCodeAndDescription;
...
function template_GET200(object, testData, url) {
  if(testData.t!=undefined && testData.t2==undefined && testData.t3==undefined)
  {
    var options = {headers: config.header,qs:{cardBrandCode:testData.t.cardBrandCode}};
  }
  else if(testData.t==undefined && testData.t2!=undefined && testData.t3!=undefined){
    var options = {headers: config.header,qs:{cardBrandCode:testData.t2.cardBrandCode, cardBrandCode:testData.t3.cardBrandCode}};
  }
  //console.log("URL  :"+url);
  ...
  var queries = null;
  queries = Query.generateDynamicQuery(testData.t.cardBrandCode);
  ...
  if (!queries.length == 0) {
    ...
    var expected_response = Framework.dbValidator(queries,
      ...
      var i;
        ...
        res[i]['reasonCode'] = res[i]["reason_code"];
        delete res[i]['reason_code'];

        res[i]["resonCodeDesc"] = res[i]["reason_description"];
        delete res[i]['reason_description'];

  }
  //console.log("---------------EXPECTED----------------"+JSON.stringify(res));
} else {
  ...
  {
  var queries1 = null;
  queries1 = Query.generateDynamicQuery1(testData.t2.cardBrandCode,testData.t3.cardBrandCode);
  //console.log("Queries *** "+queries)
  if (!queries1.length == 0) {
   
    var expected_response = Framework.dbValidator(queries1,
      config.SQLConnectionURL,
      config.resultType, 
      path.join(__dirname, config.expected_response_root_path, "responseStructure.json"),
      path.join(__dirname, config.expected_response_root_path, "actualResponse11.json"),
...
utils.VerifyResponseBody(object,actual2,res);
}  
...
function getResponse(url, object, testData, options) {
 ...
 // console.log("URL: "+url);
  ...
  //console.log("testData.TestCaseId: "+testData.TestCaseId);
  //console.log("URL: "+url);
...
function template_GET404 (object, testData, url) {
  //console.log("We are in 404 function");

  //console.log(testData.cardBrandCode.length);
  if(testData.t!=undefined && testData.t2==undefined && testData.t3==undefined)
  {
    var options = {headers: config.header,qs:{cardBrandCode:testData.t.cardBrandCode}};
  }
