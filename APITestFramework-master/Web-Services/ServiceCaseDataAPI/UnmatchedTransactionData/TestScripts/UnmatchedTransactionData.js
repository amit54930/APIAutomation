...
var config_UnmatchedTransactionDataAPI = config.UnmatchedTransactionDataAPI;
...
//Web service URL
var root_url = config.servicecasedata_root_url + config_UnmatchedTransactionDataAPI.call_suffix_togetUnmatchedTransactions;
...
/*
  ...
  var queries = null;
  var queries2 =null;
  queries = Query.generateDynamicQuery1(testData.inputs);
  if (!queries.length == 0) {
  ...
  queries2 = Query.generateDynamicQuery2(testData.inputs);
  if (!queries2.length == 0) {
    ...
    var expected_response2= Framework.dbValidator(queries2,
      ...
      config.resultType,
        ...
        var result2=obj;
      result2.stageConfigId=parseInt(result2.stageConfigId);
      result2.latestStageInd=parseInt(result2.latestStageInd);
      result2.queueId=parseInt(result2.queueId);
      result2.disputeStageId=parseInt(result2.disputeStageId);
      result2.statusId=parseInt(result2.statusId);
  } else {
    console.log("Dynamic query generator has not generated any query! Please check configurations");
    addContext(object, "Dynamic query generator has not generated any query! Please check configurations")
  }
  var expected_response3 = [];
  expected_response3.push({
    "unmatchedCaseDetailModel": result, "caseStageDetail": result2
  });

//console.log(expected_response3);
  ...
  if (response.statusCode = 200) {
    ...
    utils.VerifyResponseBody(object, actual2, expected_response3,['transactionAmount','createdTimestamp','authCode','disputeStageName','daysRemaining','dueDate','codeDescription','discoverNetworkReferenceId','caseStageName','disputeAmount','transactionAmount','settlementBankId','cardBrandName','lastModifiedTimestamp','disputeSourceId','lastModifiedBy','posEntryMode','caseNumber','statusIndicator','caseStageId']);
