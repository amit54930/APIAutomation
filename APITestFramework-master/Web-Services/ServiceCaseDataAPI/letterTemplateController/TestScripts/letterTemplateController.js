...
var config_LetterTemplateControllerAPI= config.LetterTemplateControllerAPI;
...
//Web service URL
var root_url = config.servicecasedata_root_url+config_LetterTemplateControllerAPI.call_suffix_toFetchLetterTemplate;
...
//url=url.replace("{cardBrandCode}",testData.cardBrandCode);
url=url.replace("{languageCode}",testData.languageCode);
//console.log(url);
var options = {headers: config.header, qs:{languageCode:testData.languageCode }}
...
//console.log("Status Code Matched");
  ...
  var queries = Query.generateDynamicQuery(testData.languageCode );
 ...
 // console.log("EXPECTED: "+expected_response);
    ...
    expected_response.letterTemplateLanguageId=parseInt(expected_response.letterTemplateLanguageId);
