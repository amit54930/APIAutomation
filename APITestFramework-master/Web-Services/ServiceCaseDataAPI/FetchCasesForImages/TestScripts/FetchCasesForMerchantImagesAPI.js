...
var config_FetchCasesForImagesAPI = config.FetchCasesForImagesAPI;
...
//Web service URL
var root_url = config.servicecasedata_root_url + config_FetchCasesForImagesAPI.call_suffix_toFetchCasesForImagesAPI;
...
function template1_success(object, testData, url) {
  ...
  var queries1 = Query.generateDynamicQuery1(testData.inputs);
  var queries2 = Query.generateDynamicQuery2(testData.inputs);
  var queries3 = Query.generateDynamicQuery3(testData.inputs);
  var queries4 = Query.generateDynamicQuery4(testData.inputs);

  if (!queries1.length == 0) {
    //console.log('dataQuery :'+queries1);
    ...
    var transactionMasterId = parseInt(expected.transactionMasterId);

    expected.caseNumber = caseNumber;
    expected.transactionAmount = transactionAmount;
    expected.transactionMasterId = transactionMasterId;
    var merchantHierarchyId = parseInt(expected.merchantHeirarchyId);
    // console.log("**************    "+typeof merchantHierarchyId);
    expected.merchantHierarchyId = merchantHierarchyId;
    expected = JSON.stringify(expected);
    expected = expected.replace("acquirerReferenceNumber", "acqReferenceNumber");
    // console.log("**************  "+expected);
    expected_response1 = JSON.parse(expected);

    // console.log("______________Expected Response 1_________________________" + JSON.stringify(expected_response1));
  }//if
  else {
  ...
  if (!queries2.length == 0) {
    ...
    var matchedFailCaseId = parseInt(expected.matchedFailCaseId);
    var nextInSequence = parseInt(expected.nextInSequence);
    ...
    expected.matchedFailCaseId = matchedFailCaseId;
    expected.nextInSequence = nextInSequence;
    ...
    //console.log("______________Expected Response 2_________________________" + JSON.stringify(expected_response2));
  }//if
  else {
    //  console.log("Dynamic query generator has not generated any query! Please check configurations");
    addContext(object, "Dynamic query generator has not generated any query! Please check configurations")
  ...
  if (!queries3.length == 0) {
    ...
    //console.log("______________Expected Response 3_________________________" + JSON.stringify(expected_response3));
  }//if
  else {
    //  console.log("Dynamic query generator has not generated any query! Please check configurations");
    addContext(object, "Dynamic query generator has not generated any query! Please check configurations")
  ...
  result.push({
    "caseDetail": expected_response1, "caseStageDetail": expected_response2,
    "posEntryMode": expected_response3.posEntryMode, "currencyCode": expected_response3.currencyCode,
    "merchantNumber": expected_response4.merchantNumber
  });
  ...
  if (response.statusCode = 200) {
    ...
    // console.log("**** Expected **** " + JSON.stringify(result[0]));
    utils.VerifyResponseBody(object, actual2, result[0], ['caseDetail.lastModifiedTimestamp', 'caseDetail.createdTimestamp'
      , 'caseStageDetail.lastModifiedTimestamp', 'caseStageDetail.createdTimestamp', 'caseDetail.merchantHeirarchyId',
      'caseStageDetail.merchantRespondDueDate', 'caseStageDetail.internalRespondDueDate', 'caseStageDetail.matchedFailCaseId',
      'caseStageDetail.statusId', 'caseStageDetail.cardBrandRespondDueDate', 'caseStageDetail.representmentAmount', 'caseStageDetail.caseStageName'
      , 'currencyCode', 'caseStageDetail.latestStageInd', 'caseStageDetail.nextInSequence', 'caseStageDetail.codeDescription','caseDetail.merchantRegionCode','caseStageDetail.createdBy']);
  } else {
