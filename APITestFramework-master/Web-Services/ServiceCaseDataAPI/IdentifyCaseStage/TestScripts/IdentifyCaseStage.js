...
var config_IdentifyCaseStageAPI = config.IdentifyCaseStageAPI;
...
//Web service URL
var url = config.servicecasedata_root_url+config_IdentifyCaseStageAPI.call_suffix_toIdentifyCaseStage;
...
/*
 ...
 // console.log("EXPECTED: "+expected_response);
    obj=JSON.parse(expected_response)[0];
  ...
  if (response.statusCode = 200) {
    var actual2 = (response.body).data;
      ...
      var stageId= parseInt(expected.stageId);
      expected.stageId=stageId;

      expected = JSON.stringify(expected);
      //console.log(typeof expected);
      
      expected = expected.replace("disputeStageName","stageDesc");
      expected = expected.replace("functionDescription","functionDesc");
      expected = expected.replace("actionDescription","actionCodeDesc");
      expected = expected.replace("cardBrandName","cardSchemName");
      //expected = expected.replace("stageDesc","disputeStageName");
      
      //console.log(expected);
      
      expected_response=JSON.parse(expected);
    ...
    utils.VerifyResponseBody(object, actual2, expected_response,['isReceivedInd']);
