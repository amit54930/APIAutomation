...
var config_CasePendDocAPI = config.CasePendDocAPI;
...
eventEmitter.on("testCaseData", function (testCaseData) {
  ...
  it(execution_group, function () {
    ...
    var url = root_url + config_CasePendDocAPI.call_suffix_toFetchCasePendDocData;
...
function template_GET200(object, testData, url) {
  url = url.replace("{queueId}", testData.test.queueId);
  ...
  var queries = null;
  queries = Query.generateDynamicQuery(testData.test1);
  ...
  if (response.statusCode = 200) {
    ...
    utils.VerifyResponseBody(object, actual2, expected_response, ['createdTimestamp', 'merchantName',
      'merchantHeirarchyId', 'retreivalResponseDueDate', 'retrievalResponseDueDate']);
  }
