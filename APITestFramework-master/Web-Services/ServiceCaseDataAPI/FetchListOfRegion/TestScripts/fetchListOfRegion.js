...
var config_FetchListOfRegionAPI = config.FetchListOfRegionAPI;
...
//Web service URL
var root_url = config.servicecasedata_root_url + config_FetchListOfRegionAPI.call_suffix_tofetchListOfRegion;
...
function template1_success(object, testData, url) {
  ...
  var response = getResponse(url, object, testData);
 ...
 //console.log(url);
  ...
  if (!queries.length == 0) {
    ...
    // console.log("______________databaSE_________________________"+JSON.stringify(result));
  }//if
  else {
  ...
  if (response.statusCode = 200) {
   ...
   // console.log("~~~~~~~expected DATABASE~~~~~~~~~~~~~~~~~~"+JSON.stringify(res));
   // console.log("*******************actual response+++++++++"+JSON.stringify(actual2));
    //console.log(typeof actual2 );

    utils.VerifyResponseBody(object, actual2, result[0]);
  } else {
...
/**
   ...
   */
function getResponse(url, object, testData) {
  //  object._runnable.title = testData.TestCaseName;
  // utils.testCaseInfo(object, testData);
  addContext(object, "url :" + url);
  var response = Framework.hitWebService(testData.type, url);
