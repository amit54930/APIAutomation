...
var config_saveActiveColumnPerPage= config.saveActiveColumnPerPage;
...
eventEmitter.on("testCaseData", function (testCaseData) {
  url=root_url+config_saveActiveColumnPerPage.call_suffix_toActiveColumn;
...
function template1_success(object, testData,url) {
  ...
  var options={headers: config.header,json:testData.input,qs:testData.pageId};
...
var result;
  ...
  var queries = Query.generateDynamicQuery(testData.input.pageId);
 if (!queries.length == 0) {

   var savedColumnElementID = Framework.dbValidator(queries,
    ...
    config.resultType,
      ...
      result=JSON.parse(savedColumnElementID)[0].saved_column_element_id;
      console.log( "Active column is saved with ID as "+ result);
   }
 if (result==null)
 {
  console.log( "Unable to Active column  "+ result);
 }
}
/*
