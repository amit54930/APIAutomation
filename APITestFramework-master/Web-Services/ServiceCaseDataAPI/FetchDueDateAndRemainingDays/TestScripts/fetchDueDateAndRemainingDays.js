...
var config_fetchDueDateAndRemainingDays = config.fetchDueDateAndRemainingDays;
...
//Web service URL
var root_url = config.servicedata_root_url;
...
eventEmitter.on("testCaseData", function (testCaseData) {
  url = root_url + config_fetchDueDateAndRemainingDays.call_suffix_toDueDateAndRemainingDays;
   console.log(url)
...
function template1_success(object, testData, url) {
  ...
  caseNumber =actualResponse.content[0].caseDetail.caseNumber;

  var queries = Query.generateDynamicQuery(caseNumber);
...
if (response.statusCode = 200) {
  var actualResponse = ((response.body).data);
  dueDate= actualResponse.content[0].caseStageDetail.dueDate;
 
  if(dueDate==result.card_brand_respond_due_date)
  {
    console.log("dueDates are matched in db and repsonse for case number :"+caseNumber)
    addContext(object,"dueDates are matched in db and repsonse for case number :"+caseNumber);
    addContext(object, "Result: Test case is passed");
  }
  if(dueDate!=result.card_brand_respond_due_date)
  {
    console.log( "Due date are not matched.");
    addContext(object, "Due date are not matched.")
