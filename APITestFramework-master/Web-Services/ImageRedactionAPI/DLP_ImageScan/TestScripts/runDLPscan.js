...
var config_DLP_scanForImagesAPI = config.DLP_scanForImagesAPI;
...
//Web service URL
var root_url = config.service_image_redaction_root_url;
...
eventEmitter.on("testCaseData", function (testCaseData) {
  url = root_url+config_DLP_scanForImagesAPI.call_suffix_toRunDLPScanForImages;
...
function template1_success(object, testData, url) {
 ...
 var options = {headers:config.header, json:testData.images}
...
//console.log("@@@@@@@@failure   URL   @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ "+url);
  var options = { headers: config.header,qs:{ sourceFilePath:testData.sourceFilePath, targetFilePath:testData.targetFilePath}}
