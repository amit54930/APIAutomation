...
var config_getDocument = config.getDocument;
...
//Web service URL
var root_url = config.service_merchant_data_url;
...
eventEmitter.on("testCaseData", function (testCaseData) {
  url = root_url+config_getDocument.call_suffix_toGetDocument;
  //console.log(url);
...
function template1_success(object, testData, url) {
  ...
  url = url.replace("{id}", testData.id);
  url = url.replace("{doc_id}", testData.doc_id);
