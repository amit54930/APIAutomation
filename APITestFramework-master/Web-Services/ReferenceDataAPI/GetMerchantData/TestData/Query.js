...
module.exports.generateDynamicQuery = function(inputs){
  ...
  if (inputs["merchantNumber"] != undefined) {
    tempQuery = subquery + "merchant_number='" + inputs["merchantNumber"] + "'";
    }
  if (inputs["associate"] != undefined) {
      tempQuery = tempQuery + " AND associate='" + inputs["associate"] + "'";
      }
  if (inputs["chain"] != undefined) {
        tempQuery = tempQuery + " AND chain='" + inputs["chain"] + "'";
        }
  if (inputs["corporate"] != undefined) {
          tempQuery = tempQuery + " AND corporate='" + inputs["corporate"] + "'";
          }
 if (inputs["size"] != undefined) {
  ...
  var finalQuery = query + s+tempQuery;
