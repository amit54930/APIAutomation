...
var config_MerchantMatchGet = config.GetMerchantDataAPI;
...
function template1_success(object, testData) {
  ...
  var url = root_url + config_MerchantMatchGet.call_suffix_toFetchMerchantData ;
 ...
 // console.log("Status Code Matched");
  ...
  var queries = Query.generateDynamicQuery(testData.input);
  if (!queries.length == 0) {
...
//console.log("-------------------------------"+expected_response);
  ...
  if (response.statusCode = 200) {
    ...
    utils.VerifyResponseBody(object, actual2,expected_response,['dbaCountry']);
