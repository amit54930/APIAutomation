...
var config_GetStripOffMerchantAPI = config.GetStripOffMerchantAPI;
...
//Web service URL
var url = config.service_merchant_category_root_url + config_GetStripOffMerchantAPI.call_suffix_toGetStripOffMerchant;
...
function template1_success(object, testData, url) {
  ...
  // console.log("URL: " + url);
  var options = {headers:config.header,merchantNumber:testData.merchantNumber}

  // console.log(testData.merchantNumber);
  //console.log(options);
  ...
  var queries = Query.generateDynamicQuery(testData.merchantNumber);
  if (!queries.length == 0) {
  ...
  if (response.statusCode = 200) {
    var actual2 = (response.body).data.merchantData[0];
    var expected = expected_response;

    expected = JSON.stringify(expected);
    //console.log(typeof expected);

    expected = expected.replace("isActiveInd", "activeIndicator");
    expected_response = JSON.parse(expected);

    utils.VerifyResponseBody(object, actual2, expected_response, ['comments', 'lastModifiedTimestamp', 'createdTimestamp']);
