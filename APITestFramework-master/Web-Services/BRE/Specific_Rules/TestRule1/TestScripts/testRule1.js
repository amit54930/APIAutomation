...
var config_SpecificRuleAPI = config.SpecificRuleAPI;
var config_UpdateRuleResponseAPI = config.UpdateRuleResponseAPI;
//import query object
...
//Web service URL
var url = config.specific_rule_url+config_SpecificRuleAPI.call_suffix_toRunBRE;
console.log("URL: "+url);
...
function template1_success(object, testData, url) {
  ...
  var options = { headers: config.header, json: testData.BREInput }
 ...
 //console.log( "API Response :" + JSON.stringify(response));
  ...
  var workflowBreReturnCode = response.body.result["execution-results"].results[0].value[1]["com.globalpayments.dims.rule.common.model.RuleExecutionResponse"].workflowBreReturnCode;
  console.log ("workflowBreReturnCode ="+workflowBreReturnCode);
  
  var verbiageBreReturnCode = response.body.result["execution-results"].results[0].value[1]["com.globalpayments.dims.rule.common.model.RuleExecutionResponse"].verbiageBreReturnCode;
  console.log ("verbiageBreReturnCode ="+verbiageBreReturnCode);

  var status = response.body.result["execution-results"].results[0].value[1]["com.globalpayments.dims.rule.common.model.RuleExecutionResponse"].status;
  console.log ("status ="+status);



/**
 * Now insert BRE Rule response into DiMS case_stage_detail table
 */
  var url2 = config.servicecasedata_root_url+config_UpdateRuleResponseAPI.call_suffix_toUpdateRuleResponse;
  var options2 = { headers: config.header, json: testData.RuleResponseInput }
  //console.log( "*************** options :" + JSON.stringify(options));
  var response2 = getResponse(url2, object, testData, options2);

 //console.log( "API Response :" + JSON.stringify(response2));
  utils.VerifyStatusCode(object, testData.statusCode, response.statusCode);
  console.log("Status Code Matched");
console.log(response2.body.status.statusMsg);


var query = Query.generateDynamicQuery(testData.RuleResponseInput.caseStageId);

  if (!query.length == 0) {
    //console.log('dataQuery :'+queries1);

    var expected_response = Framework.dbValidator(query,
...
if((expected_response[0]["disposition_activity_code"]='NCV')&&(expected_response[0]["disposition_action_code"]='NMC'))
{
console.log("Activity and Action Code are updated in table successfully");
}
else{
  console.log("Activity and Action Code are NOT updated in table");
}
  }
