...
var config_fetchAssignedQueueount= config.fetchAssignedQueueount;
...
eventEmitter.on("testCaseData", function (testCaseData) {
  url=root_url+config_fetchAssignedQueueount.call_suffix_toAssignedQueueCount+"?userId="+'userID';
...
url= url.replace("userID",testData.input.userId);
...
var result3;
 ...
 var queries = Query.generateDynamicQueryForWorkableCount(testData.input.userId);
 if (!queries.length == 0) {
 ...
 var queries = Query.generateDynamicQueryForCompletedCount(testData.input.userId);
 if (!queries.length == 0) {
   ...
   var expected_response2 = Framework.dbValidator(queries,
    ...
    config.resultType,
     ...
     result2=JSON.parse(expected_response2)[0].completedCount;
   
  
 }

 var queries = Query.generateDynamicQueryForCriticalCount(testData.input.userId);
 if (!queries.length == 0) {
   ...
   var expected_response3 = Framework.dbValidator(queries,
    ...
    config.resultType,
     ...
     result3=JSON.parse(expected_response3)[0].assignedCasesCriticalCount;
...
// utils.VerifyResponseBody(object,actualResponse,ox);
