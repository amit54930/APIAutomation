...
var config_FetchEventsAuditLog= config.FetchEventsAuditLog;
...
//Web service URL
var url = config.event_manager_url+config_FetchEventsAuditLog.call_suffix_toFetchEventsAuditLog;
...
function template_GET200(object, testData, url) {
  url = url.replace("{eventTemplateName}", testData.eventTemplateName);
  // console.log("URL **** "+url);
  ...
  var queries = null;
  queries = Query.generateDynamicQuery(testData.eventTemplateName);
  ...
  if (!queries.length == 0) {
    ...
    var expected_response = Framework.dbValidator(queries,
      ...
      var i;
        ...
        res[i]['eventParamId'] = parseInt(res[i].event_parameters_id);
        delete res[i]['event_parameters_id'];

        res[i]["eventParamName"] = res[i]["event_parameter_name"];
        delete res[i]['event_parameter_name'];
...
utils.VerifyResponseBody(object,actual2,res,['eventParamValue']);
}  
