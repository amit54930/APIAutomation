...
var config_BarcodeDetailsGenerateAPI = config.BarcodeDetailsGenerateAPI;
...
function template1_success(object, testData) {
  ...
  var url = root_url + config_BarcodeDetailsGenerateAPI.call_suffix_toGenerateBarcodeDetails;
  var options = {headers:config.header, qs: {test:testData.test} }
...
function template1_failure(object, testData) {
  ...
  var url = root_url + config_BarcodeDetailsGenerateAPI.BarcodeDetailsGenerateAPI;
...
function getResponse(url, object, testData, options) {
  //console.log("787974444"+JSON.stringify(options));
  object._runnable.title = testData.TestCaseName;
  utils.testCaseInfo(object, testData);
  addContext(object, "url :" + url);
  var response = Framework.hitWebService(testData.type, url, options);
