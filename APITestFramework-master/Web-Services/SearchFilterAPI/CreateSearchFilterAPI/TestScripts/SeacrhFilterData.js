...
var config_CreateSearchFilterAPI = config.CreateSearchFilterAPI;
...
eventEmitter.on("testCaseData", function (testCaseData) {
  ...
  it(execution_group, function () {
    ...
    var url = root_url + config_CreateSearchFilterAPI.call_suffix_toInsertSearchFilterData;
...
function template_GET200(object, testData, url) {
 url=url.replace("{filterTypeId}",testData.test.filterTypeId);
 var options = { headers: config.header, json:testData.test.filter, filterTypeId:testData.test.filterTypeId}
...
function template_GET404 (object, testData, url) {
  ...
  url=url.replace("{filterTypeId}",testData.test.filterTypeId);
  var options = { headers: config.header, json: testData.test.filter, test:testData.test.filterTypeId}
  var response = getResponse(url, object, testData,options);
  utils.VerifyStatusCode(object, testData.statusCode, response.statusCode);
  //console.log("Status Code Matched");
}