...
var config_merchantPortalDataSearch= config.merchantPortalDataSearch;
...
//Web service URL
var url = config.service_search_filter_root_url+config_merchantPortalDataSearch.call_suffix_merchantPortalDataSearch;
   ...
   var expected_response = Framework.dbValidator(queries,
    ...
    config.resultType,
      ...
      var i;
        ...
        res[i]["caseAmount"] = res[i]["dispute_amount"];
        ...
        var caseAmount= parseFloat(res[i].caseAmount);
        res[i].caseAmount=caseAmount;

        res[i]["acqReferenceNumber"] = res[i]["acquirer_reference_number"];
        delete res[i]['acquirer_reference_number'];

        res[i]["authorizationCode"] = res[i]["authorization_code"];
        delete res[i]['authorization_code'];

        res[i]["caseReceivedDate"] = res[i]["case_received_date"];
        delete res[i]['case_received_date'];
        ...
        res[i]["merchantRegionCode"] = res[i]["merchant_region_code"];
        delete res[i]['merchant_region_code'];

        res[i]["transactionDate"] = res[i]["transaction_date"];
        delete res[i]['transaction_date'];

        res[i]["dueDate"] = res[i]["card_brand_respond_due_date"];
        delete res[i]['card_brand_respond_due_date'];
...
utils.VerifyResponseBody(object,actual2,res,['currencyCode','codeDescription','daysRemaining','merchantStatus','posEntryMode','mcc','transactionIdentifier','originalTransactionReferenceNumber','cardBrandRegionCode']);
  } else {
