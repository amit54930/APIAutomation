...
var config_FetchSeacrhTypeID = config.FetchSeacrhTypeID;
...
eventEmitter.on("testCaseData", function (testCaseData) {
  ...
  it(execution_group, function () {
    ...
    var url = root_url + config_FetchSeacrhTypeID.call_suffix_toFetchSeacrhTypeIdData;
...
function template_GET200(object, testData, url) {
  url = url.replace("{categoryType}", testData.input);
  // console.log("URL **** "+url);
  ...
  var queries = null;
  queries = Query.generateDynamicQuery(testData.input);
  ...
  if (!queries.length == 0) {
    ...
    var expected_response = Framework.dbValidator(queries,
      ...
      var obj=parseInt(JSON.parse(expected_response)[0].search_type_id);
